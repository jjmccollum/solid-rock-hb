...
\definefontfamily[hebrew] [rm] [Keter YG] [features={hebrew,dlig}]
...
\define[1]\StartTransMark{\enquad{\switchtobodyfont[apparatus]⸊}\doifnot{#1}{1}{{\switchtobodyfont[english]\textsuperscript{#1}}}} %reverse symbol for RTL
...
\definefontsynonym [hebrewtitle] [name:KeterAramTsova] [features={hebrew, kern=no}]
\definefont[BookTitleFont][hebrewtitle at 26pt][36bp]

%Macro for adjusting kerning of Book heading texts (can any of this can be handled in the \definefont call?):
\define[1]\BookTextCommand{%
	\kerncharacters[0.6666]%increase letterspacing
	#1%print text
}

%Macro for page break after Book headings:
\define\BookAfter{%
	\page[blankpagebreak]%
}
...
\setuphead[Book][
	...
	textstyle=\BookTitleFont,%set text style
	textcommand=\BookTextCommand,%use the command defined above to handle the typesetting
	after=\BookAfter%add a double page break
]
...
\define[1]\Verse{%
	...
	\doifelse{\getvariable{text}{verse}}{0}{%
		...
		\marking[Verse]{#1}{\switchtobodyfont[english, 11pt]\textdir TLT\bf \getvariable{text}{chapter}\nobreak\,:\nobreak\,#1}\nobreak\enquad%
	}{%
		%Otherwise, just typeset the verse:
		\marking[Verse]{#1}{\switchtobodyfont[english, 11pt]\textdir TLT\bf #1}\nobreak\enquad%
	}%
...
\setupheadertexts[\rlap{\switchtobodyfont[english, 11pt]\textdir TLT\RefRange}\hfill {\switchtobodyfont[hebrew, 13pt]\kerncharacters[0.25]\textdir TRT\ReplaceHeaderSpaces{\structureuservariable{booktitle}}} \hfill][][][\hfill {\switchtobodyfont[hebrew, 13pt]\kerncharacters[0.25]\textdir TRT\ReplaceHeaderSpaces{\structureuservariable{booktitle}}} \hfill \llap{\switchtobodyfont[english, 11pt]\textdir TLT\RefRange}] %even left, even right, odd left, odd right
